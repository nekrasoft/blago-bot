...
- Работает только в группах из `WHITELIST_CHAT_IDS`; в остальных пишет, что доступ запрещен, и выходит из чата.
...
- `WHITELIST_CHAT_IDS` - список разрешенных `chat_id` групп через запятую (например: `-1001234567890,-1009876543210`).
...
4. Добавьте `chat_id` этой группы в `WHITELIST_CHAT_IDS`, иначе бот выйдет из чата.
...
- `WHITELIST_CHAT_IDS` обязателен. Если список пустой/невалидный, бот не запустится.
