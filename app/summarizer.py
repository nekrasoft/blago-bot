...
class TenderSummarizer:
    ...
    async def _chunk_summary(self, chunk: str, idx: int, total: int) -> str:
        prompt = (
            ...
            "взяты контакты\n"
            "10) Факты для рекомендации: рассчитанная цена машины 30 м3, город/район фактического "
            "исполнения работ, признаки городов Киров, Кирово-Чепецк/К. Чепецк, Зуевка/Зеувка "
            "или Зуевского района\n"
            "11) Ключевые риски/неясности"
        )
    ...
    async def _final_summary(self, content: str, file_name: str | None = None) -> str:
        ...
        prompt = (
            ...
            "Для блока 'Рекомендация' укажи ровно один итоговый вариант и причину только по правилам ниже:\n"
            "- ОТКАЗ, низкая цена — если рассчитанная цена машины 30 м3 меньше 50 000 руб.\n"
            "- ОТКАЗ, конфликт интересов — если территориальность: город Киров или город Кирово-Чепецк "
            "(также учитывай написания К. Чепецк, К-Чепецк).\n"
            "- УЧАСТВОВАТЬ — если территориальность: Зуевка/Зеувка или Зуевский район.\n"
            "- Отсутствует — во всех остальных случаях или если данных недостаточно.\n"
            "Если одновременно выполняются условия отказа и участия, выбери ОТКАЗ и укажи причину отказа.\n"
