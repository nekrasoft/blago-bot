...
class TenderSummarizer:
    ...
    async def _chunk_summary(self, chunk: str, idx: int, total: int) -> str:
        prompt = (
            ...
            "4) Сроки/этапы, включая КРАЙНИЙ СРОК ПОДАЧИ ЗАЯВКИ (дата и время)\n"
    ...
    async def _final_summary(self, content: str, file_name: str | None = None) -> str:
        ...
        prompt = (
            ...
            "Для блока 'Ключевые сроки' обязательно укажи:\n"
            "- крайний срок подачи заявки на участие в тендере\n"
            "- точную дату/время и часовой пояс (если указаны)\n"
            "- источник в документе (раздел/пункт)\n"
            "Если срок подачи заявки не найден, так и напиши: "
            "'Крайний срок подачи заявки не найден в документах'.\n"
